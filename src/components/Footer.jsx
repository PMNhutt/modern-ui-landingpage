          ...
          <div key={links.key} className="flex flex-col ss:my-0 my-4 min-w-[150px]">
            ...
            <ul className="mt-5">
              {links.links.map((link, index) => (
                ...
                leading-[24px] text-dimWhite hover:text-secondary cursor-pointer ${index !== links.links.length - 1 ? 'mb-4' : 'mb-0'}`}>
                  {link.name}
                </li>
              ))}
            </ul>
      ...
      <div className="flex flex-row md:mt-0 mt-6">
        {socialMedia.map((media, index) => (
          <img
            ...
            src={media.icon}
            alt={media.id}
            className={`w-[21px] h-[21px] object-contain cursor-pointer ${index !== media.length - 1 ? 'mr-6' : 'mr-0'}`} />
        ))}
      </div>
    </div>
  </section>
